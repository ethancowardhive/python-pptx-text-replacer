...
class TextReplacer:
        ...
        if len(slides.strip()) == 0:
            ...
            for rr in re.split("\\s*,\\s*", slides.strip()):
                ...
                if low is None or high is None:
                    raise ValueError(
                        'Slide list (--slides "%s") is not a comma separated list of slide numbers (i.e. 1) or slide number ranges (i.e. 4-12)'
                        % (slides)
                    )
                if low < 1 or low > slide_cnt:
                    raise ValueError(
                        'Slide number %s in list (--slides "%s") is lower than 1 or bigger than the last slide number %s'
                        % (low, slides, slide_cnt)
                    )
                if high < 1 or high > slide_cnt:
                    raise ValueError(
                        'Slide number %s in list (--slides "%s") is lower than 1 or bigger than the last slide number %s'
                        % (high, slides, slide_cnt)
                    )
                if low > high:
                    raise ValueError(
                        'Slide range %s in list (--slides "%s") is invalid.'
                        % (r, slides)
                    )
    ...
    def replace_text(
            ...
            for i in range(0, len(self._replacements) - 1):
                ...
                for j in range(i + 1, len(self._replacements)):
                    ...
                    if repl_i.find(srch_j) >= 0:
                        self._write_warning(
                            "Replacement string %s at index %s matches search string %s at index %s. This may produce unintended results due to chained replacements!"
                            % (repl_i, i, srch_j, j)
                        )
                    if srch_j.find(srch_i) >= 0:
                        if srch_j.replace(srch_i, repl_i) == repl_j:
                            self._write_warning(
                                "Match/Replacement ('%s','%s') at index %s is obsolete due to match/replacement ('%s','%s') at index %s"
                                % (srch_j, repl_j, j, srch_i, repl_i, i)
                            )
                        else:
                            self._write_warning(
                                "Match/Replacement ('%s','%s') at index %s will never match due to match/replacement ('%s','%s') at index %s"
                                % (srch_j, repl_j, j, srch_i, repl_i, i)
                            )
        ...
        if len(self._messages) > 0:
            print(
                "The following warnings and errors have been issued during this run:",
                file=sys.stderr,
            )
    ...
    def _write_warning(self, msg: str):
        text = "WARNING: " + msg
    ...
    def _replace_text_in_text_frame(self, level, shape, text_frame):
        for srch, replacement in self._replacements:
            text = "\n".join(
                "".join(self._ensure_unicode(run.text) for run in par.runs)
            ...
            while pos_in_text_frame >= 0:
                if self._verbose:
                    print(
                        "%sTrying to match '%s' -> matched at %s%s"
                        % (
                            "  " * level,
                            srch,
                            pos_in_text_frame,
                ...
                for paragraph in text_frame.paragraphs:
                    para_text = "".join(
                        self._ensure_unicode(run.text) for run in paragraph.runs
                    )
                    ...
                    if pos_in_paragraph >= paragraph_len:
                        ...
                        (to_match, to_replace) = self._replace_runs_text(
                            level + 1,
                            shape,
                            paragraph_idx,
                            paragraph.runs,
                ...
                if self._use_regex:
                    ...
                    pos_in_text_frame = text.find(
                        srch, pos_in_text_frame + to_replace_len
                    )
    ...
    def _replace_runs_text(
        self, level, shape, paragraph_idx, runs, pos, srch, replacement
    ):
        cnt = len(runs)
        i = 0
        while i < cnt:
                ...
                while i < cnt:
                    ...
                    if pos + match_len < olen:
                        ...
                        if self._verbose:
                            print(
                                "%sRun[%s,%s]: '%s' -> '%s'"
                                % ("  " * level, paragraph_idx, i, otext, run.text)
                            )
                        elif not self._quiet:
                            print(
                                "Slide[%s].%s[id=%s].Run[%s,%s]: '%s' -> '%s'"
                                % (
                                    self._current_slide_idx + 1,
                                    str(shape.shape_type)[
                                        0 : str(shape.shape_type).find(" ")
                                    ],
                                    shape.shape_id,
                                    paragraph_idx,
                                    i,
                    ...
                    if self._verbose:
                        print(
                            "%sRun[%s,%s]: '%s' -> '%s'"
                            % ("  " * level, paragraph_idx, i, otext, ntext)
                        )
                    elif not self._quiet:
                        print(
                            "Slide[%s].%s[id=%s].Run[%s,%s]: '%s' -> '%s'"
                    ...
                    to_match = to_match[
                        part_match_len:
                    ]  # this is what is left to match
                    match_len -= (
                        part_match_len  # this is the length of the match that is left
                    )
    ...
    def _process_text_frame(self, level, shape, text_frame):
        if self._verbose:
            print(
                "%sTextFrame: '%s'"
                % ("  " * level, self._make_printable(text_frame.text))
            )
            paragraph_idx = 0
            for paragraph in text_frame.paragraphs:
                print(
                    ...
                    % (
                        "  " * (level + 1),
                        paragraph_idx,
                        self._make_printable(paragraph.text),
                    )
                )
                run_idx = 0
    ...
    def _process_shapes(self, level, shape_list_parent):
        for shape in shape_list_parent.shapes:
            if self._verbose:
                print(
                    "%sShape[%s, id=%s, type=%s]"
                    % (
                        "  " * level,
                        shape_list_parent.shapes.index(shape),
                        shape.shape_id,
                        shape.shape_type,
                    )
                )
                ...
                if self._charts:
                    ...
                    for category in chart.plots[0].categories:
                        if self._verbose:
                            print(
                                "%sCategory[%s] '%s'"
                                % ("  " * (level + 2), category_idx, category)
                            )
                        for srch, replace in self._replacements:
                            if self._use_regex:
                                changed_category = re.sub(
                                    srch, replace, category, flags=re.MULTILINE
                                )
                            else:
                            ...
                            if changed_category == category:
                                if self._verbose:
                                    print(
                                        "%sReplacing '%s' -> no match"
                                        % ("  " * (level + 3), srch)
                                    )
                            else:
                                if self._verbose:
                                    print(
                                        "%sReplacing '%s' -> changed to '%s'"
                                        % ("  " * (level + 3), srch, changed_category)
                                    )
                                elif not self._quiet:
                                    print(
                                        "Slide[%s].%s[id=%s].Category[%s]: replacing '%s' -> '%s' changed to '%s'"
                                        % (
                                            self._current_slide_idx + 1,
                                            str(shape.shape_type)[
                                                0 : str(shape.shape_type).find(" ")
                    ...
                    if categories_changed:
                        ...
                        try:
                            ...
                            self._write_error(
                                "Replacing chart data of chart with id %s on slide %s failed with error: %s"
                                % (
                                    shape.shape_id,
                                    self._current_slide_idx,
                                    str(err.args[0]),
                                )
                            )
                elif self._verbose:
...
"""
    ...
    if len(ns.matches) != len(ns.replacements):
        print(
            "There must be as many match-strings (-m) as there are replacement-strings (-r)",
            file=sys.stderr,
        )
        return 1
